...
function replaceTheme(theme, project) {
  console.log(
    `Setting ${chalk.green(project)} theme to ${chalk.green(
      theme
    )}. Files changed:`
  );
  try {
    replace({
      regex: /(?:@[^ \/]+\/)?doca-([-_a-z0-9]+)-theme/gi,
      replacement: theme,
      recursive: true,
      paths: [path.join(project, 'src'), path.join(project, 'webpack')]
    });
  } catch (err) {
    console.error(
      chalk.red(
        `Error: Can't find dirs ${path.join(project, 'src')} and ${path.join(
          project,
          'webpack'
        )}`
      )
...
function installTheme(theme, project) {
  ...
  exec(`npm install ${theme} --save`, { cwd: project }, function(err, stdout) {
    if (err) {
      ...
      console.log(
        chalk.green(
          `Theme ${theme} at ${project} has been successfully installed!`
        )
      );
